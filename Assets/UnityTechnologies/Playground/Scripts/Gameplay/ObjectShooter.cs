...
public class ObjectShooter : MonoBehaviour
{
    ...
    Vector3 spawnPositionOffset = new Vector3(0f, 0f, 0f);
	...
	void Update ()
	{
		...
		{
			Vector2 actualBulletDirection = (relativeToRotation) ? (Vector2)(Quaternion.Euler(0, transform.eulerAngles.y, transform.eulerAngles.z) * shootDirection) : shootDirection;

			GameObject newObject = Instantiate<GameObject>(prefabToSpawn);

            newObject.transform.position = this.transform.position + spawnPositionOffset;
